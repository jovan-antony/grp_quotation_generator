...
curl -v http://192.168.0.10:8000/ 2>&1 | grep -E "HTTP|status|message|Connection"
echo ""

echo "2. Testing health endpoint (/health):"
curl -v http://192.168.0.10:8000/health 2>&1 | grep -E "HTTP|status|Connection"
echo ""

echo "3. Testing API endpoint (/api/companies):"
curl -v http://192.168.0.10:8000/api/companies 2>&1 | head -20
echo ""
