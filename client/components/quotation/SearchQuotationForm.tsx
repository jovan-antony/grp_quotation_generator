...
export default function SearchQuotationForm({
  ...
  const generateQuotationPreview = (quotation: any, tanksData: any, termsData?: any, leftSig?: {name:string;title:string;mobile:string;email:string;signatureImage?:string}, rightSig?: {name:string;title:string;mobile:string;email:string;signatureImage?:string}, fromCompanyName?: string) => {
    ...
    const savedTankBrand = (quotation.additionalData?.tankBrand || '').toUpperCase();
    const brandName = (companyCode === 'GRP' && savedTankBrand === 'COLEX')
      ? 'COLEX KOREA'
          ...
          <CardContent className="pt-3 px-6">
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {quotations.map((quotation) => (
                ...
                >
                  {/* Quote No as a full-width row */}
                  <div className="mb-1 text-sm font-semibold text-blue-900 truncate" style={{lineHeight:'1.2'}}>
                    Quote No: <span className="break-all">{quotation.full_main_quote_number}</span>
                  </div>
                  <div className="flex items-start justify-between gap-4">

                    <div
                      className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-2 text-sm cursor-pointer"
                      onClick={() => handleSelectQuotation(quotation)}
                    >
                      <div className="min-w-0">                        <span className="font-semibold">Recipient:</span>{' '}
                        <span className="break-words">{quotation.recipient_name}</span>
                      </div>
                      <div className="min-w-0">
                        <span className="font-semibold">Company:</span>{' '}
                        <span className="break-words">{quotation.recipient_company}</span>
                      </div>
                      <div className="min-w-0">
